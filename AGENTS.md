...
**系统概要**：自托管多用户 AI Agent 系统。输入：飞书/Telegram/QQ/Web。执行：Docker 容器或宿主机进程。主 Agent 通过 Codex CLI 的 `Codex -p` runner 运行，Memory Agent 仍使用 Codex Agent SDK。输出：Web 流式推送 + IM 显式发送。
...
**新增 StreamEvent**：`shared/stream-event.ts` 加类型 → `make sync-types` → `container/agent-runner/src/providers/Codex/Codex-stream-processor.ts` 或对应 provider 发射 → `web/src/stores/chat.ts` 处理
